...
_compare:
	; rax == 64, eax == 32, ax == 16, ah == al == 8 == 1byte
	; al은 ax의 하위 비트 즉, ax : 0001100100111100 라고하면, ah : 00011001, al: 00111100
	sub rax, rbx ; al과 bl을 빼게 되면 8비트이기 때문에 오버플로우가 발생할 수 있음, 8비트 레지스터를 사용함에 유의. (https://man-25-1.tistory.com/60)
	xor rbx, rbx ; rbx를 사용시 함수 호출 규약(콜러와 콜리)에 따라 처음 값으로 되돌려 놓아야 한다.??
	ret
